...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // WICHTIG: Button bleibt gesperrt, während wir laden!
    if(startBtn) {
        startBtn.innerText = "Verbindung zur DB wird aufgebaut...";
...
async function ladeDatenAusSupabase() {
    try {
        ...
        // Erst wenn ALLES geladen ist, schalten wir den Button frei!
        if(startBtn && quizQuestions.length > 0) {
            startBtn.innerText = "Jetzt Quiz starten";
            startBtn.disabled = false;
        } else {
            throw new Error("Keine Fragen zurückgegeben.");
        }
    } catch(err) {
        console.error("Ladefehler:", err);
        if(startBtn) {
            startBtn.innerText = "Fehler: Prüfe die Browser-Konsole (F12)";
...
function startQuiz() {
    // Doppelter Boden: Verhindert das Überspringen, falls die Liste leer ist
    if (!quizQuestions || quizQuestions.length === 0) {
        alert("Daten noch nicht geladen. Bitte warte einen Moment oder lade die Seite neu.");
        return;
    }
...
if(closeModalBtn) closeModalBtn.addEventListener('click', () => { if(contactModal) contactModal.classList.add('hidden'); });// 1. Supabase Verbindung aufsetzen
const SUPABASE_URL = "https://kqqzxhiylxfjgxkrvpd.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtxcXp4a2hpeWx4ZmpneGtydnBkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAzMDY2NjUsImV4cCI6MjA5NTg4MjY2NX0.4qyuvNLniTnvPtiLgs41M1YnaCc6g8PeiE0bVXFuwKU";
const supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
